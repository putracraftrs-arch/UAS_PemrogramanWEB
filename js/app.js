...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const JADWAL = [
    { film: 'Jumbo',            genre: 'Animation / Adventure', durasi: '102 menit', tanggal: '2026-06-02', labelTgl: 'Selasa, 2 Jun 2026',  poster: 'Images/jumbo-indonesian-movie-poster.jpg',      alt: 'Jumbo' },
    { film: 'Ancika: 1995',     genre: 'Romance / Drama',       durasi: '100 menit', tanggal: '2026-06-03', labelTgl: 'Rabu, 3 Jun 2026',     poster: 'Images/ancika-1995.jpg',                        alt: 'Ancika: 1995' },
    { film: 'A Minecraft Movie', genre: 'Adventure / Fantasy',  durasi: '101 menit', tanggal: '2026-06-04', labelTgl: 'Kamis, 4 Jun 2026',    poster: 'Images/a-minecraft-movie-movie-poster.jpg',     alt: 'A Minecraft Movie' },
    { film: 'Agak Laen',        genre: 'Horror / Comedy',       durasi: '119 menit', tanggal: '2026-06-05', labelTgl: 'Jumat, 5 Jun 2026',    poster: 'Images/agak-laen.jpg',                          alt: 'Agak Laen' },
    { film: 'Siksa Kubur',      genre: 'Horror / Drama',        durasi: '117 menit', tanggal: '2026-06-06', labelTgl: 'Sabtu, 6 Jun 2026',    poster: 'Images/siksa-kubur.jpg',                        alt: 'Siksa Kubur' },
    { film: 'Mission: Impossible \u2013 The Final Reckoning', genre: 'Action / Thriller', durasi: '169 menit', tanggal: '2026-06-07', labelTgl: 'Minggu, 7 Jun 2026', poster: 'Images/25MIFR.jpg', alt: 'Mission: Impossible \u2013 The Final Reckoning' },
    { film: 'Avengers: Infinity War', genre: 'Action / Sci-Fi', durasi: '149 menit', tanggal: '2026-06-08', labelTgl: 'Senin, 8 Jun 2026',   poster: 'Images/infinity.jpg',                           alt: 'Avengers: Infinity War' },
  ];
...
(function () {
  ...
  const FILMS = {
    'Jumbo': {
      genre: 'Animation / Adventure', durasi: '102 menit', poster: 'Images/jumbo-indonesian-movie-poster.jpg',
      jadwal: [
    ...
    'Ancika: 1995': {
      genre: 'Romance / Drama', durasi: '100 menit', poster: 'Images/ancika-1995.jpg',
      jadwal: [
    ...
    'A Minecraft Movie': {
      genre: 'Adventure / Fantasy', durasi: '101 menit', poster: 'Images/a-minecraft-movie-movie-poster.jpg',
      jadwal: [
    ...
    'Agak Laen': {
      genre: 'Horror / Comedy', durasi: '119 menit', poster: 'Images/agak-laen.jpg',
      jadwal: [
    ...
    'Siksa Kubur': {
      genre: 'Horror / Drama', durasi: '117 menit', poster: 'Images/siksa-kubur.jpg',
      jadwal: [
    ...
    'Mission: Impossible \u2013 The Final Reckoning': {
      genre: 'Action / Thriller', durasi: '169 menit', poster: 'Images/25MIFR.jpg',
      jadwal: [
    ...
    'Avengers: Infinity War': {
      genre: 'Action / Sci-Fi', durasi: '149 menit', poster: 'Images/infinity.jpg',
      jadwal: [
  ...
  let selFilm = null, selCinema = null, selTanggal = null, selJam = null;
  let selSeats = [], seatState = {};
  ...
  function selectFilm(nama) {
    selFilm = nama; selCinema = null; selTanggal = null; selJam = null; selSeats = [];
    const data = FILMS[nama];
    const poster2 = document.getElementById('fiPoster2');
    if (poster2) { poster2.src = data.poster; poster2.onerror = function() { this.src='https://placehold.co/60x84/120a02/fb923c?text=Film'; }; }
    setText('fiTitle2', nama); setText('fiGenre2', data.genre); setText('fiDurasi2', '⏱ ' + data.durasi);
    renderCinemas();
    goToStep(2);
  }
  ...
  function renderCinemas() {
    ...
    showEl('dateCard', false); showEl('timeCard', false); updateSeatButton();
  }

  function selectCinema(cinemaId) {
    selCinema = CINEMAS.find(c => c.id === cinemaId); selTanggal = null; selJam = null;
    document.querySelectorAll('.cinema-card').forEach(c => c.classList.toggle('active', c.dataset.id === cinemaId));
    renderDateStrip();
    showEl('dateCard', true); showEl('timeCard', false); updateSeatButton();
  ...
  function selectTanggal(j, chip) {
    selTanggal = j; selJam = null;
    document.querySelectorAll('.date-chip').forEach(c => c.classList.remove('active'));
    if (chip && chip.classList) chip.classList.add('active');
    renderTimeGrid(j); showEl('timeCard', true); updateSeatButton();
  ...
  function renderTimeGrid(j) {
    ...
    j.sesi.forEach(jam => {
      const btn = document.createElement('button');
      btn.type = 'button'; btn.className = 'time-btn'; btn.textContent = jam;
  ...
  function renderSeatGrid() {
    ...
    allTickets.forEach(t => {
      if (t.film === selFilm && t.cinemaId === cinema.id && t.tanggal === selTanggal.tanggal && t.jam === selJam && Array.isArray(t.kursi)) {
        t.kursi.forEach(s => { if (seatState[s] !== undefined) seatState[s] = 'taken'; });
      }
    });
    ...
    ROWS.forEach(rowLetter => {
      ...
      lbl.className = 'row-label'; lbl.textContent = rowLetter;
      row.appendChild(lbl);
      const halfCols = Math.floor(cinema.cols / 2);
      for (let c = 1; c <= cinema.cols; c++) {
        if (c === halfCols + 1) { const gap = document.createElement('div'); gap.className = 'seat-gap'; row.appendChild(gap); }
        const seatId = rowLetter + c;
        const seat = document.createElement('button');
        seat.type = 'button'; seat.className = 'seat ' + seatState[seatId]; seat.dataset.seatId = seatId; seat.title = seatId;
  ...
  function toggleSeat(btn, seatId) {
    ...
    if (idx === -1) {
      if (selSeats.length >= 6) { showToast('⚠️ Maksimal 6 kursi per pemesanan', '#b45309'); return; }
      selSeats.push(seatId); btn.className = 'seat selected';
    } else {
      selSeats.splice(idx, 1); btn.className = 'seat ' + seatState[seatId];
    }
    updateSeatCounter();
  }
  ...
  function updateSeatCounter() {
    ...
    if (!counter) return;
    if (selSeats.length === 0) { counter.textContent = 'Pilih kursi kamu'; return; }
  ...
  function goToSeatStep() {
    if (!selFilm || !selCinema || !selTanggal || !selJam) { showToast('Pilih bioskop, tanggal, dan jam tayang dulu.', '#b45309'); updateSeatButton(); return; }
    const data = FILMS[selFilm];
    const sPoster = document.getElementById('sSumPoster');
    if (sPoster) { sPoster.src = data.poster; sPoster.onerror = function() { this.src='https://placehold.co/60x84/120a02/fb923c?text=Film'; }; }
    ...
    selSeats = []; renderSeatGrid(); goToStep(3);
  }

  function goToBookingForm() {
    const data = FILMS[selFilm];
    const bPoster = document.getElementById('bsStripPoster');
    if (bPoster) { bPoster.src = data.poster; bPoster.onerror = function() { this.src='https://placehold.co/42x58/120a02/fb923c?text=Film'; }; }
    ...
    if (typeof AUTH !== 'undefined') {
      ...
      if (user) {
        const namaEl = document.getElementById('nama'); const emailEl = document.getElementById('email');
  ...
  function closeAuthRequired() {
    const overlay = document.getElementById('authRequiredOverlay');
    const modal   = document.getElementById('authRequiredModal');
    if (overlay) { overlay.classList.remove('show'); overlay.style.cssText = ''; }
    if (modal)   { modal.classList.remove('show');   modal.style.cssText   = ''; }
    document.body.style.overflow = '';
  }
  ...
  function initBooking() {
    ...
    document.getElementById('btnBackToSeats')     ?.addEventListener('click', () => { selSeats = []; goToStep(3); renderSeatGrid(); });

    document.getElementById('btnToSeats')?.addEventListener('click', () => {
      if (!selCinema || !selTanggal || !selJam) { showToast('Pilih bioskop, tanggal, dan jam tayang dulu.', '#b45309'); updateSeatButton(); return; }
      goToSeatStep();
    });

    document.getElementById('btnToBookingForm')?.addEventListener('click', () => {
      if (selSeats.length === 0) { showToast('⚠️ Pilih minimal 1 kursi', '#b45309'); return; }
      goToBookingForm();
    });
    ...
    document.getElementById('bookingForm')?.addEventListener('submit', function(e) {
      ...
      if (typeof AUTH === 'undefined' || !AUTH.getSession) { showToast('Sistem login belum siap.', '#b45309'); return; }
      const session = AUTH.getSession();
      const email   = (session?.email || document.getElementById('email').value).trim().toLowerCase();
      if (!session) { openAuthRequired(); return; }
      if (!nama)    { setErr('nama', 'Nama wajib diisi.'); return; }
      const tiket = {
        id: Date.now(), nama, email,
        film: selFilm, cinemaId: selCinema.id, cinema: selCinema.nama, tipe: selCinema.tipe,
        tanggal: selTanggal.tanggal, jam: selJam,
        kursi: [...selSeats], jumlah: selSeats.length,
        hargaPerKursi: selCinema.harga, total: selSeats.length * selCinema.harga,
      };
      const semua = JSON.parse(localStorage.getItem('cinego_tiket') || '[]');
      semua.push(tiket);
      localStorage.setItem('cinego_tiket', JSON.stringify(semua));
      [1,2,3,4].forEach(i => { const si = document.getElementById('si' + i); if (si) { si.classList.remove('active'); si.classList.add('done'); } });
      showToast('✅ Tiket berhasil dipesan!', '#15803d');
      setTimeout(() => { window.location.href = 'MyTicket.html'; }, 1900);
    });

    document.getElementById('btnCloseAuthRequired')?.addEventListener('click', closeAuthRequired);
    document.getElementById('authRequiredOverlay') ?.addEventListener('click', e => { if (e.target === e.currentTarget) closeAuthRequired(); });
    ...
    document.getElementById('linkBkToDaftar')?.addEventListener('click', e => {
      e.preventDefault();
      document.querySelectorAll('#authRequiredModal .auth-tab-btn').forEach(b => b.classList.toggle('active', b.dataset.tab === 'daftar'));
      document.querySelectorAll('#authRequiredModal .auth-panel').forEach(p => p.classList.toggle('active', p.id === 'bkPanel-daftar'));
    });
    document.getElementById('linkBkToLogin')?.addEventListener('click', e => {
      e.preventDefault();
      document.querySelectorAll('#authRequiredModal .auth-tab-btn').forEach(b => b.classList.toggle('active', b.dataset.tab === 'login'));
      document.querySelectorAll('#authRequiredModal .auth-panel').forEach(p => p.classList.toggle('active', p.id === 'bkPanel-login'));
    });

    document.getElementById('toggleBkLoginPw')?.addEventListener('click', () => {
      const inp = document.getElementById('bkLoginPw'); if (!inp) return;
      inp.type = inp.type === 'password' ? 'text' : 'password';
      document.getElementById('toggleBkLoginPw').textContent = inp.type === 'password' ? '👁' : '🙈';
    });
    document.getElementById('toggleBkRegPw')?.addEventListener('click', () => {
      const inp = document.getElementById('bkRegPw'); if (!inp) return;
      inp.type = inp.type === 'password' ? 'text' : 'password';
      document.getElementById('toggleBkRegPw').textContent = inp.type === 'password' ? '👁' : '🙈';
    });
    ...
    document.getElementById('bkLoginForm')?.addEventListener('submit', e => {
      ...
      emailErr?.classList.remove('show'); pwErr?.classList.remove('show');
      if (!EMAIL_RE.test(email)) { emailErr.textContent = 'Email tidak valid.'; emailErr.classList.add('show'); return; }
      if (!pw)                   { pwErr.textContent = 'Password wajib diisi.'; pwErr.classList.add('show'); return; }
      const result = AUTH.login(email, pw);
      if (!result.ok) {
        if (result.msg.includes('Email')) { emailErr.textContent = result.msg; emailErr.classList.add('show'); }
        else { pwErr.textContent = result.msg; pwErr.classList.add('show'); }
        return;
      }
      closeAuthRequired(); goToSeatStep();
    });
    ...
    document.getElementById('bkRegisterForm')?.addEventListener('submit', e => {
      ...
      [namaErr, emailErr, pwErr, confErr].forEach(el => el?.classList.remove('show'));
      let ok = true;
      if (nama.length < 2)       { namaErr.textContent  = 'Nama minimal 2 karakter.';  namaErr.classList.add('show');  ok = false; }
      if (!EMAIL_RE.test(email)) { emailErr.textContent = 'Email tidak valid.';          emailErr.classList.add('show'); ok = false; }
      if (pw.length < 8)         { pwErr.textContent    = 'Password minimal 8 karakter.';pwErr.classList.add('show');   ok = false; }
      if (pw !== confirm)        { confErr.textContent  = 'Password tidak cocok.';       confErr.classList.add('show'); ok = false; }
      if (!ok) return;
      const result = AUTH.register({ nama, email, password: pw });
      if (!result.ok) { emailErr.textContent = result.msg; emailErr.classList.add('show'); return; }
      closeAuthRequired(); goToSeatStep();
    });
    ...
    if (urlFilm && FILMS[urlFilm]) {
      ...
      if (urlTanggal && urlJam) {
        const jadwalEntry = FILMS[urlFilm].jadwal.find(j => j.tanggal === urlTanggal);
        if (jadwalEntry) {
          ...
          setTimeout(() => document.getElementById('btnToSeats')?.scrollIntoView({ behavior:'smooth', block:'nearest' }), 200);
          return;
        }
      }
      setTimeout(() => document.getElementById('step2')?.scrollIntoView({ behavior:'smooth', block:'start' }), 200);
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const FILMS_TR = [
    { id:1, film:'Siksa Kubur',    genre:'Horror / Drama',       tags:['Horror','Drama'],             durasi:117, rating:8.2, tahun:2024, sutradara:'Joko Anwar',          pemain:'Faradina Mufti, Reza Rahadian, Slamet Rahardjo',                       sinopsis:'Sita adalah seorang wanita ateis yang keras kepala. Untuk membuktikan bahwa siksa kubur hanyalah isapan jempol belaka, ia memutuskan mengalaminya sendiri — dan apa yang ia temukan di sana mengubah segalanya.',                                                              poster:'Images/siksa-kubur.jpg',                    bookingKey:'Siksa+Kubur',                                              baseVotes:1240 },
    { id:2, film:'A Minecraft Movie', genre:'Adventure / Fantasy', tags:['Adventure','Fantasy','Family'], durasi:101, rating:7.8, tahun:2025, sutradara:'Jared Hess',       pemain:'Jack Black, Jason Momoa, Jennifer Coolidge',                           sinopsis:'Empat orang tersedot ke dalam dunia balok bernama Overworld dan harus berjuang untuk bertahan hidup. Satu-satunya harapan mereka adalah seorang pahlawan tak terduga bernama Steve.',                                                                                      poster:'Images/a-minecraft-movie-movie-poster.jpg',  bookingKey:'A+Minecraft+Movie',                                        baseVotes:980  },
    { id:3, film:'Agak Laen',      genre:'Horror / Comedy',      tags:['Horror','Comedy'],             durasi:119, rating:8.5, tahun:2024, sutradara:'Muhadkly Acho',       pemain:'Bene Dion, Oki Rengga, Dimas Anggara, Indra Jegel',                    sinopsis:'Empat sahabat pengelola rumah hantu ketiban sial saat dukun sewaan mereka tewas di dalam wahana. Sosok sang dukun pun mulai gentayangan, membuat terror nyata bagi mereka sendiri.',                                                                                         poster:'Images/agak-laen.jpg',                      bookingKey:'Agak+Laen',                                                baseVotes:870  },
    { id:4, film:'Avengers: Infinity War', genre:'Action / Sci-Fi', tags:['Action','Sci-Fi','Marvel'], durasi:149, rating:8.4, tahun:2018, sutradara:'Anthony & Joe Russo', pemain:'Robert Downey Jr., Chris Evans, Chris Hemsworth, Scarlett Johansson',   sinopsis:'Thanos berusaha mengumpulkan enam Infinity Stones untuk memusnahkan setengah populasi alam semesta. Para Avengers bersatu dalam pertempuran paling epik untuk menghentikannya.',                                                                                             poster:'Images/infinity.jpg',                       bookingKey:'Avengers%3A+Infinity+War',                                 baseVotes:760  },
    { id:5, film:'Jumbo',          genre:'Animation / Adventure', tags:['Animation','Family','Adventure'], durasi:102, rating:8.0, tahun:2025, sutradara:'Ryan Andriandhy', pemain:'Angga Yunanda, Prilly Latuconsina',                                     sinopsis:'Si kecil Don bersahabat dengan sosok misterius bernama Jumbo. Petualangan ajaib mereka membawa pelajaran berharga tentang persahabatan, keberanian, dan makna keluarga yang sesungguhnya.',                                                                                 poster:'Images/jumbo-indonesian-movie-poster.jpg',  bookingKey:'Jumbo',                                                    baseVotes:620  },
    { id:6, film:'Ancika: 1995',   genre:'Romance / Drama',      tags:['Romance','Drama'],             durasi:100, rating:7.9, tahun:2023, sutradara:'Benni Setiawan',      pemain:'Marthino Lio, Raisa Andriana',                                          sinopsis:'Kisah cinta Dilan dan Ancika di Bandung tahun 1995. Dilan harus membuktikan dirinya kepada gadis yang memiliki kepribadian kuat, mandiri, dan tidak mudah takluk oleh rayuan.',                                                                                            poster:'Images/ancika-1995.jpg',                    bookingKey:'Ancika%3A+1995',                                           baseVotes:510  },
    { id:7, film:'Mission: Impossible \u2013 The Final Reckoning', genre:'Action / Thriller', tags:['Action','Thriller'], durasi:169, rating:8.3, tahun:2025, sutradara:'Christopher McQuarrie', pemain:'Tom Cruise, Hayley Atwell, Simon Pegg, Ving Rhames', sinopsis:'Ethan Hunt menghadapi ancaman terbesar sepanjang kariernya — sebuah AI bernama The Entity yang mampu mengendalikan seluruh persenjataan nuklir global. Ini misi terakhir, dengan taruhan tertinggi.', poster:'Images/25MIFR.jpg', bookingKey:'Mission%3A+Impossible+%E2%80%93+The+Final+Reckoning', baseVotes:445 },
  ];
  ...
  let activeGenre = 'Semua', activeSort = 'votes', searchQuery = '', viewMode = 'grid', openFilmId = null;

  function getVoteCounts() { return JSON.parse(localStorage.getItem(KEY_VOTES) || '{}'); }
  function getVotedList()  { return JSON.parse(localStorage.getItem(KEY_VOTED) || '[]'); }
  function hasVoted(id)    { return getVotedList().includes(id); }

  function castVote(id) {
    if (hasVoted(id)) return false;
    const counts = getVoteCounts(); counts[id] = (counts[id] || 0) + 1; localStorage.setItem(KEY_VOTES, JSON.stringify(counts));
    const voted = getVotedList(); voted.push(id); localStorage.setItem(KEY_VOTED, JSON.stringify(voted));
    return true;
  }

  function totalVotes(id) {
    const film = FILMS_TR.find(f => f.id === id); if (!film) return 0;
    return film.baseVotes + (getVoteCounts()[id] || 0);
  }

  function getWishlist() {
    const session = AUTH.getSession(); if (!session) return [];
    return (JSON.parse(localStorage.getItem(KEY_WL) || '{}'))[session.email] || [];
  }
  function inWishlist(id) { return getWishlist().includes(id); }

  function toggleWishlist(id) {
    const session = AUTH.getSession();
    if (!session) { showToastTR('🔒 Login terlebih dahulu untuk menggunakan fitur Wishlist.'); return; }
    const email = session.email;
    const all = JSON.parse(localStorage.getItem(KEY_WL) || '{}');
    const wl  = all[email] || [];
    if (wl.includes(id)) { all[email] = wl.filter(x => x !== id); showToastTR('💔 Dihapus dari wishlist.'); }
    else                 { all[email] = [...wl, id];               showToastTR('❤️ Ditambahkan ke wishlist!'); }
  ...
  function fmtVotes(n) { return n >= 1000 ? (n/1000).toFixed(1).replace(/\.0$/,'') + 'k' : n.toLocaleString('id-ID'); }
  function fmtDur(min) { const h = Math.floor(min/60), m = min%60; return h ? `${h}j ${m}m` : `${m} menit`; }
  function rankBadgeClass(r) { return r===1?'rank-1':r===2?'rank-2':r===3?'rank-3':'rank-other'; }
  function rankLabel(r)      { return r===1?'🥇 #1':r===2?'🥈 #2':r===3?'🥉 #3':`#${r}`; }
  function listRankClass(r)  { return r===1?'list-rank-1':r===2?'list-rank-2':r===3?'list-rank-3':'list-rank-other'; }
  function escHtmlTR(s)      { return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }

  function buildGenrePills() {
    const all  = new Set(); FILMS_TR.forEach(f => f.tags.forEach(t => all.add(t)));
    const tags = ['Semua', ...Array.from(all).sort()];
    const wrap = document.getElementById('genrePills'); wrap.innerHTML = '';
    tags.forEach(tag => {
      const btn = document.createElement('button');
      btn.className = 'genre-pill' + (tag === activeGenre ? ' active' : '');
      btn.textContent = tag;
      btn.addEventListener('click', () => { activeGenre = tag; wrap.querySelectorAll('.genre-pill').forEach(b => b.classList.remove('active')); btn.classList.add('active'); render(); });
  ...
  function getFilteredSorted() {
    let list = [...FILMS_TR];
    if (activeGenre !== 'Semua') list = list.filter(f => f.tags.includes(activeGenre));
    if (searchQuery) { const q = searchQuery.toLowerCase(); list = list.filter(f => f.film.toLowerCase().includes(q)||f.genre.toLowerCase().includes(q)||f.sutradara.toLowerCase().includes(q)||f.pemain.toLowerCase().includes(q)); }
    switch (activeSort) {
  ...
  function renderStats() {
    ...
    const top   = [...FILMS_TR].sort((a,b) => totalVotes(b.id) - totalVotes(a.id))[0];
  ...
  function renderGrid(films) {
    const wrap = document.createElement('div'); wrap.className = 'trend-grid';
    films.forEach((f, idx) => {
      const rank = idx+1, votes = totalVotes(f.id), voted = hasVoted(f.id), wled = inWishlist(f.id);
  ...
  function renderList(films) {
    const wrap = document.createElement('div'); wrap.className = 'trend-list';
    films.forEach((f, idx) => {
      const rank = idx+1, votes = totalVotes(f.id), voted = hasVoted(f.id), wled = inWishlist(f.id);
      const encImg = encodeURIComponent(f.film.slice(0,12));
      const item = document.createElement('div'); item.className = 'trend-list-item';
      item.innerHTML = `
  ...
  function renderEmpty() {
    const div = document.createElement('div'); div.className = 'trend-empty';
  ...
  function bindEvents(container) {
    container.querySelectorAll('.btn-vote:not([disabled])').forEach(btn => {
      btn.addEventListener('click', e => { e.stopPropagation(); const id = parseInt(btn.dataset.id); if (castVote(id)) { showToastTR('🔥 Vote berhasil! Terima kasih sudah memilih.'); render(); if (openFilmId === id) populateModal(id); } });
    });
    ...
    container.querySelectorAll('.btn-detail-overlay, .btn-list-detail').forEach(btn => {
      btn.addEventListener('click', e => { e.stopPropagation(); openModalTR(parseInt(btn.dataset.id)); });
    });
  }
  ...
  function render() {
    ...
    renderStats();
    const container = document.getElementById('trendContainer'); container.innerHTML = '';
    if (films.length === 0) { container.appendChild(renderEmpty()); return; }
    const view = viewMode === 'grid' ? renderGrid(films) : renderList(films);
    container.appendChild(view); bindEvents(container);
  }

  function populateModal(id) {
    const f = FILMS_TR.find(f => f.id === id); if (!f) return;
    const votes = totalVotes(id), voted = hasVoted(id), wled = inWishlist(id);
    const encImg = encodeURIComponent(f.film.slice(0,14));
    const mPoster = document.getElementById('mPoster');
    mPoster.src = f.poster; mPoster.alt = f.film; mPoster.onerror = function() { this.src = `https://placehold.co/100x142/120a02/fb923c?text=${encImg}`; };
    ...
    document.getElementById('mPills').innerHTML = `<span class="mpill mpill-genre">${escHtmlTR(f.genre)}</span><span class="mpill mpill-year">📅 ${f.tahun}</span><span class="mpill mpill-dur">⏱ ${fmtDur(f.durasi)}</span>`;
    ...
    btnVote.onclick     = () => { if (castVote(id)) { showToastTR('🔥 Vote berhasil!'); populateModal(id); render(); } };
  ...
  function openModalTR(id) {
    openFilmId = id; populateModal(id);
  ...
  function closeModalTR() {
    ...
    document.body.style.overflow = ''; openFilmId = null;
  }

  document.getElementById('btnModalClose').addEventListener('click', closeModalTR);
  document.getElementById('filmModalOverlay').addEventListener('click', e => { if (e.target === e.currentTarget) closeModalTR(); });
  document.addEventListener('keydown', e => { if (e.key === 'Escape') closeModalTR(); });

  document.getElementById('btnViewGrid').addEventListener('click', () => { viewMode='grid'; document.getElementById('btnViewGrid').classList.add('active'); document.getElementById('btnViewList').classList.remove('active'); render(); });
  document.getElementById('btnViewList').addEventListener('click', () => { viewMode='list'; document.getElementById('btnViewList').classList.add('active'); document.getElementById('btnViewGrid').classList.remove('active'); render(); });
  document.getElementById('trendSearch').addEventListener('input', e => { searchQuery = e.target.value.trim(); render(); });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initAuthGateMT() {
    document.querySelectorAll('.auth-tab-btn').forEach(btn => btn.addEventListener('click', () => AUTH.switchAuthTab(btn.dataset.tab)));
    document.getElementById('gateLinkToDaftar')?.addEventListener('click', e => { e.preventDefault(); AUTH.switchAuthTab('daftar'); });
    document.getElementById('gateLinkToLogin') ?.addEventListener('click', e => { e.preventDefault(); AUTH.switchAuthTab('login'); });
    document.getElementById('toggleGLoginPw')  ?.addEventListener('click', () => { const inp = document.getElementById('gLoginPw'); inp.type = inp.type==='password'?'text':'password'; document.getElementById('toggleGLoginPw').textContent = inp.type==='password'?'👁':'🙈'; });
    document.getElementById('toggleGRegPw')    ?.addEventListener('click', () => { const inp = document.getElementById('gRegPw');   inp.type = inp.type==='password'?'text':'password'; document.getElementById('toggleGRegPw').textContent   = inp.type==='password'?'👁':'🙈'; });
    ['gLoginEmail','gLoginPw','gRegNama','gRegEmail','gRegPw','gRegPwConfirm'].forEach(id => document.getElementById(id)?.addEventListener('input', () => clearErr(id)));
    ...
    document.getElementById('gateLoginForm')?.addEventListener('submit', e => {
      ...
      if (!ok) return;
      const result = AUTH.login(email, pw);
      if (!result.ok) { if(result.msg.includes('Email')) setErr('gLoginEmail',result.msg); else setErr('gLoginPw',result.msg); return; }
      window.location.reload();
    });

    document.getElementById('gateRegisterForm')?.addEventListener('submit', e => {
      e.preventDefault();
      const nama=document.getElementById('gRegNama').value.trim(), email=document.getElementById('gRegEmail').value.trim();
      const pw=document.getElementById('gRegPw').value, confirm=document.getElementById('gRegPwConfirm').value;
      let ok=true;
  ...
  function getAllData()    { return JSON.parse(localStorage.getItem(KEY) || '[]'); }
  function getUserEmail() { return (AUTH.getSession()?.email || '').toLowerCase(); }
  function getData()      { const email = getUserEmail(); return getAllData().filter(t => (t.email||'').toLowerCase() === email); }
  function saveData(arr)  { localStorage.setItem(KEY, JSON.stringify(arr)); }
  function formatRpMT(n)  { return 'Rp ' + n.toLocaleString('id-ID'); }
  function formatTglMT(s) { if(!s) return '-'; const[y,m,d]=s.split('-'); const bln=['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des']; return `${d} ${bln[parseInt(m)-1]} ${y}`; }
  function escHtmlMT(str) { return String(str).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
  function badgeClass(tipe) { if(tipe==='VIP') return 'badge-vip'; if(tipe==='IMAX') return 'badge-imax'; if(tipe==='4DX') return 'badge-4dx'; return 'badge-regular'; }
  function showToastMT(msg, duration=2200) { const t=document.getElementById('toast'); t.textContent=msg; t.classList.add('show'); setTimeout(()=>t.classList.remove('show'),duration); }
  ...
  function closeModal() { document.getElementById('modalOverlay').classList.remove('show'); confirmCallback = null; }

  document.getElementById('btnCancelModal').addEventListener('click', closeModal);
  document.getElementById('modalOverlay')  .addEventListener('click', e => { if(e.target===e.currentTarget) closeModal(); });
  document.getElementById('btnConfirmModal').addEventListener('click', () => { if(confirmCallback) confirmCallback(); closeModal(); });
  ...
  function renderTable() {
    ...
    const filtered = semua.filter(t => t.nama.toLowerCase().includes(filter)||t.film.toLowerCase().includes(filter)||(t.email&&t.email.toLowerCase().includes(filter)));
    ...
    `;
    if (semua.length === 0) { wrapper.style.display='none'; empty.style.display='block'; stats.style.display='none'; return; }
    wrapper.style.display=''; empty.style.display='none'; stats.style.display='flex';
    ...
    filtered.forEach((t, idx) => {
      ...
      const kursiStr  = Array.isArray(t.kursi) ? t.kursi.join(', ') : (t.kursi||'-');
    ...
    body.querySelectorAll('.btn-hapus').forEach(btn => {
      btn.addEventListener('click', () => {
        const id = parseInt(btn.dataset.id); const tiket = getData().find(t => t.id === id);
        showModal('Hapus Tiket?', `Hapus tiket "${tiket?tiket.film:''}" atas nama ${tiket?tiket.nama:''}?`, () => { saveData(getAllData().filter(t => t.id !== id)); renderTable(); showToastMT('🗑️ Tiket berhasil dihapus.'); });
      });
    });
  }
  ...
  document.getElementById('btnHapusSemua').addEventListener('click', () => {
    const semua = getData();
    if (semua.length === 0) { showToastMT('Belum ada tiket untuk dihapus.'); return; }
    showModal('Hapus Semua Tiket?', `Semua ${semua.length} pesanan akan dihapus permanen.`, () => {
      const email = getUserEmail(); saveData(getAllData().filter(t => (t.email||'').toLowerCase() !== email)); renderTable(); showToastMT('🗑️ Semua tiket telah dihapus.');
    });
  });

  document.getElementById('searchInput').addEventListener('input', e => { currentFilter = e.target.value; renderTable(); });
  renderTable();
});


/* ─────────────────────────────────────────────────────────────
   6. MY ACCOUNT  ·  MyAccount.html
   ───────────────────────────────────────────────────────────── */
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showToastMA(msg) { const t=document.getElementById('toast'); t.textContent=msg; t.className='toast'; t.classList.add('show'); setTimeout(()=>t.classList.remove('show'),2400); }
  function validateFieldMA(id, cond, errId) { const el=document.getElementById(id); const er=document.getElementById(errId); if(!cond(el)){el.classList.add('error');er.classList.add('show');return false;}el.classList.remove('error');er.classList.remove('show');return true; }
  function clearErrMA(id) { document.getElementById(id)?.classList.remove('error'); document.getElementById('err-'+id)?.classList.remove('show'); }
  function setErrMA(id, msg) { const el=document.getElementById(id); const err=document.getElementById('err-'+id); if(el) el.classList.add('error'); if(err){err.textContent=msg;err.classList.add('show');} }
  function formatRpMA(n) { return 'Rp ' + n.toLocaleString('id-ID'); }
  function escHtmlMA(s) { return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
  function formatTglMA(s) { if(!s) return '-'; const[y,m,d]=s.split('-'); const bln=['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agu','Sep','Okt','Nov','Des']; return `${d} ${bln[parseInt(m)-1]} ${y}`; }
  function getTicketsForCurrentUser() { const email=(AUTH.getSession()?.email||'').toLowerCase(); return JSON.parse(localStorage.getItem(KEY_TIKET_MA)||'[]').filter(t=>(t.email||'').toLowerCase()===email); }
  ...
  function initAuthGateMA() {
    ...
    document.getElementById('toggleGLoginPw')  ?.addEventListener('click', () => { const inp=document.getElementById('gLoginPw'); inp.type=inp.type==='password'?'text':'password'; document.getElementById('toggleGLoginPw').textContent=inp.type==='password'?'👁':'🙈'; });
    document.getElementById('toggleGRegPw')    ?.addEventListener('click', () => { const inp=document.getElementById('gRegPw');   inp.type=inp.type==='password'?'text':'password'; document.getElementById('toggleGRegPw').textContent=inp.type==='password'?'👁':'🙈'; });
    ['gLoginEmail','gLoginPw','gRegNama','gRegEmail','gRegPw','gRegPwConfirm'].forEach(id => document.getElementById(id)?.addEventListener('input', () => clearErrMA(id)));

    document.getElementById('gateLoginForm')?.addEventListener('submit', e => {
      e.preventDefault();
      const email=document.getElementById('gLoginEmail').value.trim(), pw=document.getElementById('gLoginPw').value;
      let ok=true;
      if(!EMAIL_RE_MA.test(email)){setErrMA('gLoginEmail','Email tidak valid.');ok=false;}
      if(!pw){setErrMA('gLoginPw','Password wajib diisi.');ok=false;}
      if(!ok) return;
      const result=AUTH.login(email,pw);
      if(!result.ok){if(result.msg.includes('Email'))setErrMA('gLoginEmail',result.msg);else setErrMA('gLoginPw',result.msg);return;}
      window.location.reload();
    });
    ...
    document.getElementById('gateRegisterForm')?.addEventListener('submit', e => {
      ...
      const pw=document.getElementById('gRegPw').value, confirm=document.getElementById('gRegPwConfirm').value;
      let ok=true;
      if(nama.length<2)       {setErrMA('gRegNama','Nama minimal 2 karakter.');    ok=false;}
      if(!EMAIL_RE_MA.test(email)){setErrMA('gRegEmail','Email tidak valid.');       ok=false;}
      if(pw.length<8)         {setErrMA('gRegPw','Password minimal 8 karakter.');  ok=false;}
      if(pw!==confirm)        {setErrMA('gRegPwConfirm','Password tidak cocok.');   ok=false;}
      if(!ok) return;
      const result=AUTH.register({nama,email,password:pw});
      if(!result.ok){setErrMA('gRegEmail',result.msg);return;}
      window.location.reload();
    });
  }
  ...
  function initAccountPage() {
    ...
    function updateAvatar() {
      const u=AUTH.getCurrentUser()||{}, nama=u.nama||'Pengguna CineGo';
    ...
    const fields = { acNama:'nama', acUsername:'username', acEmail:'email', acTelp:'telp', acTgl:'tgl', acKota:'kota' };
    Object.entries(fields).forEach(([elId,key]) => { const el=document.getElementById(elId); if(el&&user[key]) el.value=user[key]; });
    const genderSel=document.getElementById('acGender'); if(genderSel&&user.gender) genderSel.value=user.gender;
    ...
    document.getElementById('btnLogout')?.addEventListener('click', () => { if(confirm('Yakin ingin keluar?')) AUTH.logout(); });

    ['acNama','acEmail'].forEach(id => document.getElementById(id)?.addEventListener('input', () => clearErrMA(id)));

    document.getElementById('profilForm')?.addEventListener('submit', e => {
      e.preventDefault();
      let ok=true;
      if(!validateFieldMA('acNama', el=>el.value.trim()!=='','err-acNama'))   ok=false;
      if(!validateFieldMA('acEmail',el=>EMAIL_RE_MA.test(el.value.trim()),'err-acEmail')) ok=false;
      if(!ok) return;
      ...
      if(!result?.ok){setErrMA('acEmail',result?.msg||'Profil gagal disimpan.');return;}
      updateAvatar(); showToastMA('✅ Profil berhasil disimpan!');
    });

    document.getElementById('btnResetProfil')?.addEventListener('click', () => document.getElementById('profilForm').reset());
    ...
    ['pwLama','pwBaru','pwKonfirm'].forEach(id => document.getElementById(id)?.addEventListener('input', () => clearErrMA(id)));

    document.getElementById('pwForm')?.addEventListener('submit', e => {
      e.preventDefault();
      const lama=document.getElementById('pwLama').value, baru=document.getElementById('pwBaru').value, konfirm=document.getElementById('pwKonfirm').value;
      let ok=true;
      if(!lama)           {setErrMA('pwLama','Password lama wajib diisi.');   ok=false;}
      if(baru.length<8)   {setErrMA('pwBaru','Password minimal 8 karakter.'); ok=false;}
      if(baru!==konfirm)  {setErrMA('pwKonfirm','Password tidak cocok.');      ok=false;}
      if(!ok) return;
      const result=AUTH.changePassword(lama,baru);
      if(!result.ok){setErrMA('pwLama',result.msg);return;}
      document.getElementById('pwForm').reset(); showToastMA('🔒 Password berhasil diubah!');
    });

    document.getElementById('btnDeleteAccount')?.addEventListener('click', () => { if(!confirm('Hapus akun secara permanen? Semua data akan hilang.')) return; AUTH.deleteAccount(); });
  }
  ...
  function renderRiwayat() {
    ...
    body.innerHTML = '';
    if (!semua.length) { wrap.style.display='none'; empty.style.display='block'; return; }
    wrap.style.display=''; empty.style.display='none';
    semua.forEach((t,i) => {
      const tipe=t.tipe||'Regular', jumlah=t.jumlah||(Array.isArray(t.kursi)?t.kursi.length:1);
      const badgeCls=tipe==='VIP'?'badge-vip':tipe==='IMAX'?'badge-imax':tipe==='4DX'?'badge-4dx':'badge-regular';
      const tr=document.createElement('tr');
      tr.innerHTML=`
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ['cnNama','cnEmail','cnTopik','cnPesan'].forEach(id => {
    ...
    ['input','change'].forEach(ev => {
      el.addEventListener(ev, () => { el.classList.remove('error'); document.getElementById('err-'+id)?.classList.remove('show'); });
    });
  });

  function validateFieldCU(id, condition, errId) {
    const el=document.getElementById(id); const err=document.getElementById(errId);
    if(!condition(el)){el.classList.add('error');err?.classList.add('show');return false;}
    el.classList.remove('error');err?.classList.remove('show');return true;
  }

  function showToastCU(msg) { const t=document.getElementById('toast'); if(!t) return; t.textContent=msg; t.classList.add('show'); setTimeout(()=>t.classList.remove('show'),2500); }
  ...
  document.getElementById('contactForm').addEventListener('submit', function(e) {
    ...
    const valid = [
      ...
      validateFieldCU('cnEmail', el=>emailRe.test(el.value.trim()),'err-cnEmail'),
      validateFieldCU('cnTopik', el=>el.value!=='',              'err-cnTopik'),
      validateFieldCU('cnPesan', el=>el.value.trim().length>=10, 'err-cnPesan'),
    ].every(Boolean);
    if (!valid) return;
    const pesan = {
      id   : Date.now(),
