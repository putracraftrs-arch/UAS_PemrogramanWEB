document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!AUTH.getSession()) {
    ...
    initAuthGate(); // FIX: panggil sebelum return agar form login/daftar berfungsi
    return;
  }
  ...
  function badgeClass(tipe) {
    // fallback: di Booking.js tidak ada tipe VIP, tapi tetap dukung tiket lama dari data sebelumnya
  ...
  function renderTable() {
    ...
    stats.innerHTML = `
      ...
      <div class="stat-pill"><span class="sp-label">Total Belanja</span><span class="sp-value">${formatRp(totalHarga)}</span></div>
    `;
    ...
    if (filtered.length === 0) {
      body.innerHTML = `<tr><td colspan="11" style="text-align:center;color:#5a3a20;padding:32px">
        Tidak ada hasil untuk "<strong style="color:#a87050">${escHtml(currentFilter)}</strong>"
      </td></tr>`;
      return;
    }
    ...
    filtered.forEach((t, idx) => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <div class="film-name">${escHtml(t.nama)}</div>
          <div class="email-txt">${escHtml(t.email || '')}</div>
        </td>
        <td>${escHtml(t.film)}</td>
        <td>${formatTgl(t.tanggal)}</td>
        <td>${escHtml(t.jam)}</td>
        <td style="color:#5a3a20;font-size:.78rem;max-width:120px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"
            title="${escHtml(cinemaStr)}">${escHtml(cinemaStr)}</td>
        <td style="color:#a87050;font-size:.78rem;max-width:80px">${escHtml(kursiStr)}</td>
        <td><span class="badge ${badgeClass(tipe)}">${escHtml(tipe)}</span></td>
        <td style="text-align:center">${jumlah}</td>
        <td class="harga-cell">${formatRp(t.total || 0)}</td>
        <td>
          <button class="btn-hapus" data-id="${t.id}">Hapus</button>
        </td>
      `;
      body.appendChild(tr);
    });
  ...
  function hapusSatu(id) {
    ...
    renderTable();
    showToast('🗑️ Tiket berhasil dihapus.');
  }

  // ── Hapus semua ───────────────────────────────────────────────────────────
  document.getElementById('btnHapusSemua').addEventListener('click', () => {
    const semua = getData();
    if (semua.length === 0) { showToast('Belum ada tiket untuk dihapus.'); return; }
    showModal(
      'Hapus Semua Tiket?',
      `Semua ${semua.length} pesanan akan dihapus permanen.`,
      () => {
        const email = getUserEmail();
        saveData(getAllData().filter(t => (t.email || '').toLowerCase() !== email));
        renderTable();
        showToast('🗑️ Semua tiket telah dihapus.');
      }
    );
  });
